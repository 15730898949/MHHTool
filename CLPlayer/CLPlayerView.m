...
- (BOOL)isPlay{
    if (self.state == CLPlayerStateBuffering || self.state == CLPlayerStatePlaying) {
        return YES;
    }else{
        return NO;
    }
...
- (void)setState:(CLPlayerState)state{
    ...
    if (state == CLPlayerStateBuffering) {
        ...
        NSLog(@"加载失败");
#endif
    }else if(state == CLPlayerStatePlaying){
        [self.maskView.loadingView stopAnimation];
        if (_isUserPlay) {
            [self playVideo];
        }
...
- (void)appDidEnterPlayground:(NSNotification *)note{
    //继续播放
    if (_isUserPlay && !_isEnd && self.configure.backPlay) {
        [self playVideo];
    }
}
