...
- (void)configSubviews {
    ...
    UITapGestureRecognizer *tapges = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(showMaskView)];
...
- (void)showMaskView{
    ...
    _playButton.hidden = _maskView.hidden;
}
...
- (void)layoutSubviews {
    ...
    _playButton.frame = CGRectMake(20, [TZCommonTools tz_statusBarHeight] + 44 + 10, 30, 30);
    _playButton.tz_centerX = self.contentView.tz_centerX;
    _playButton.tz_centerY = self.contentView.tz_centerY;
    _iCloudErrorIcon.frame = CGRectMake(20, [TZCommonTools tz_statusBarHeight] + 44 + 10, 28, 28);
    _iCloudErrorLabel.frame = CGRectMake(53, [TZCommonTools tz_statusBarHeight] + 44 + 10, self.tz_width - 63, 28);
    
    _maskView.frame = CGRectMake(0, self.tz_height - 64-60, self.tz_width, 40);
    _progress.frame = CGRectMake(20, 19, self.tz_width-40, 3);
}
...
-(void)panAction:(UIPanGestureRecognizer *)pan
{
    ...
    CGPoint point=[pan locationInView:_maskView];
    ...
    //NSLog(@"调整进度！");
    //进度调整幅度计算
    double xx = point1.x / (_maskView.frame.size.width);
    float progress;
    //通过监测手势状态来执行分阶段的赋值，取值，以及方法的执行 pan.state
    ...
    //NSLog(@"progress = %.3f, xx = %0.3f", progress, xx);
//    mCurPostion = progress * [self.myPlayer getDuration];
    
//    _progress.progress = (double)mCurPostion / mDuration;
//    self.lblTimeJinDu.text = [Utilities timeToHumanString:mCurPostion];
    NSLog(@"-----%f",progress);
    if (pan.state == UIGestureRecognizerStateEnded)
    {
//        [self.player seekTo:mCurPostion];
    }
